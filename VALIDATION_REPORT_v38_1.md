...
3. HTML이 `/static/app.js?v=38.2.0`, `/static/style.css?v=38.2.0`을 참조하는지 확인
...
`/api/inspect`가 성공한 뒤 UI 표시 코드에서 예외가 발생하더라도 서버의 성공 결과를 실패로 오판하지 않아야 한다. v38.2은 서버 응답 성공 상태를 DOM 렌더링보다 먼저 커밋하며, UI 표시 오류는 비치명적 진단으로 분리한다.
